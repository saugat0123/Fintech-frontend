...
export class SmeLoanSummaryComponent implements OnInit, OnDestroy {
  ...
  getLoanDataHolder() {
    ...
    if (!ObjectUtil.isEmpty(this.loanDataHolder.creditRiskGradingAlpha)) {
      ...
      const crgParsedData = JSON.parse(
        this.loanDataHolder.creditRiskGradingAlpha.data
      );
      ...
      this.creditRiskAlphaScore =
        ObjectUtil.isEmpty(crgParsedData.totalScore) ||
        Number.isNaN(Number(crgParsedData.totalScore))
          ? 0
          : crgParsedData.totalScore;
      if (
        this.creditRiskGrade === 'Excellent' ||
        this.creditRiskGrade === 'Very Good'
      ) {
    ...
    if (!ObjectUtil.isEmpty(this.loanDataHolder.creditRiskGradingLambda)) {
      const crgParsedData = JSON.parse(
        this.loanDataHolder.creditRiskGradingLambda.data
      );
