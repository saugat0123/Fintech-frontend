    ...
    if (this.formData['selectedArray'] !== undefined) {
      if (this.formData['initialForm'] !== undefined) {
        ...
        if (this.apartmentSelected) {
          apartmentDetail.forEach((d) => {
            this.getSecurityAndFixedCollateralData(d.uuid, d, 'Building Security');
          });
        }
        if (this.plantSelected) {
          plantDetail.forEach((d) => {
            this.getSecurityAndFixedCollateralData(d.uuid, d, 'Plant and Machinery Security');
          });
        }
      }
    }
