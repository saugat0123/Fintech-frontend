...
export class NetTradingAssetsComponent implements OnInit {
    ...
    ngOnInit() {
        this.buildNtaForm();
        if (!ObjectUtil.isEmpty(this.netTradingAssetsData.data)) {
            this.netTradingAssetsForm.patchValue(JSON.parse(this.netTradingAssetsData.data));
        }
        this.getFiscalYears();
    }
    ...
    getFiscalYears() {
        this.fiscalYearService.getAll().subscribe(response => {
            this.fiscalYearArray = [(response.detail as Array<FiscalYear>)[0]];
        }, error => {
            console.log(error);
        });
    }
