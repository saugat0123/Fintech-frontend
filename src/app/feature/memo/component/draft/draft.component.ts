...
@Component({
    selector: 'app-memo-draft',
    templateUrl: './draft.component.html',
    styles: []
})
export class DraftComponent implements OnInit {

    static TITLE = `${MemoBaseComponent.TITLE} - Draft`;
    ...
    static loadData(other: DraftComponent) {
        other.spinner = true;
        if (other.search === null || other.search === undefined) {
            other.search = `stage=DRAFT&sentBy.id=${LocalStorageUtil.getStorage().userId}`;
        }
    ...
    ngOnInit() {
        this.breadcrumbService.notify(DraftComponent.TITLE);
        DraftComponent.loadData(this);
    }
