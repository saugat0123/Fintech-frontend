...
export class UIComponent implements OnInit, DoCheck {
  ...
  public checkAll(args): void {
    if (args === 'initial') {
      for (let i = 0; i < this.initialDocumentList.length; i++) {
        this.initialDocumentList[i].checked = true;
        this.finalInitialDocument.push(this.initialDocumentList[i]);
      }
    }
    if (args === 'renew') {
      for (let i = 0; i < this.renewalDocumentList.length; i++) {
        this.renewalDocumentList[i].checked = true;
        this.finalRenewalDocument.push(this.renewalDocumentList[i]);
      }
    }
    if (args === 'closure') {
      for (let i = 0; i < this.closureDocumentList.length; i++) {
        this.closureDocumentList[i].checked = true;
        this.finalClosureDocument.push(this.closureDocumentList[i]);
      }
    }
    if (args === 'eligibility') {
      for (let i = 0; i < this.eligibilityDocumentList.length; i++) {
        this.eligibilityDocumentList[i].checked = true;
        this.finalEligibilityDocument.push(this.eligibilityDocumentList[i]);
      }
    }
    if (args === 'enhance') {
      for (let i = 0; i < this.enhanceDocumentList.length; i++) {
        this.enhanceDocumentList[i].checked = true;
        this.finalEnhanceDocument.push(this.enhanceDocumentList[i]);
      }
    }
    if (args === 'partialSettlement') {
      for (let i = 0; i < this.partialSettlementDocumentList.length; i++) {
        this.partialSettlementDocumentList[i].checked = true;
        this.finalPartialSettlementDocument.push(this.partialSettlementDocumentList[i]);
      }
    }
    if (args === 'fullSettlement') {
      for (let i = 0; i < this.fullSettlementDocumentList.length; i++) {
        this.fullSettlementDocumentList[i].checked = true;
        this.finalFullSettlementDocument.push(this.fullSettlementDocumentList[i]);
      }
    }
    if (args === 'cadDocument') {
      for (let i = 0; i < this.cadDocumentUploadList.length; i++) {
        this.cadDocumentUploadList[i].checked = true;
        this.finalCadDocumentUploadList.push(this.cadDocumentUploadList[i]);
      }
    }
    if (args === 'renewWithEnhancement') {
      for (let i = 0; i < this.renewWithEnhancementDocumentList.length; i++) {
        this.renewWithEnhancementDocumentList[i].checked = true;
        this.finalRenewWithEnhancementDocument.push(this.renewWithEnhancementDocumentList[i]);
      }
    }
  }
}
