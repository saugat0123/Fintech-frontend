...
export class LoanDeedProprietorshipComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (!ObjectUtil.isEmpty(this.initialInfo)) {
            ...
            this.loanDeedProprietorship.patchValue(this.initialInfo);
        } else {
            this.addTableData();
            this.fillForm();
        }
    ...
    fillForm() {
        this.loanDeedProprietorship.patchValue({
        ...
        this.loanDeedProprietorship.get
        (['loanData', 0, 'loanFacilityType']).patchValue(this.nepData.miscellaneousDetail.loanFacilityTypeInNep);
        this.loanDeedProprietorship.get(['loanData', 0, 'amount2']).patchValue(this.nepData.miscellaneousDetail.loanAmountInFig);
        this.loanDeedProprietorship.get(['loanData', 0, 'amountInWords2']).patchValue(this.nepData.miscellaneousDetail.loanAmountInWord);
    }

    addTableData() {
        (this.loanDeedProprietorship.get('loanData') as FormArray).push(
    ...
    setTableData(data) {
        const formArray = this.loanDeedProprietorship.get('loanData') as FormArray;
        if (data.length === 0) {
            this.addTableData();
            return;
        }
    ...
    removeLoanDetail(index) {
        (this.loanDeedProprietorship.get('loanData') as FormArray).removeAt(index);
    }
    ...
    changeToNepAmount1(event: any, i, formArrayName) {
        this.loanDeedProprietorship.get([formArrayName, i, 'amountInWords2']).patchValue(event.nepVal);
        this.loanDeedProprietorship.get([formArrayName, i, 'amount2']).patchValue(event.val);
    }

    patchFunction1(formArrayName, i, formControlName) {
        const patchValue1 = this.loanDeedProprietorship.get([formArrayName, i, formControlName]).value;
        return patchValue1;
    }
}
