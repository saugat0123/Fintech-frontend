...
@Component({
    selector: 'app-vehicle-loan-deed-individual',
    templateUrl: './vehicle-loan-deed-individual.component.html',
    styleUrls: ['./vehicle-loan-deed-individual.component.scss']
})
export class VehicleLoanDeedIndividualComponent implements OnInit {
    @Input() cadData: CustomerApprovedLoanCadDocumentation;
    @Input() documentId: number;
    @Input() customerLoanId: number;
    vehicleLoanDeedIndividual: FormGroup;
    nepData;
    initialInfo;
    ...
    ngOnInit() {
        ...
        if (!ObjectUtil.isEmpty(this.cadData) && !ObjectUtil.isEmpty(this.cadData.cadFileList)) {
            this.cadData.cadFileList.forEach(singleCadFile => {
                if (singleCadFile.customerLoanId === this.customerLoanId && singleCadFile.cadDocument.id === this.documentId) {
                    this.vehicleLoanDeedIndividual.patchValue(JSON.parse(singleCadFile.initialInformation));
                }
            });
        }
        ...
        if (!ObjectUtil.isEmpty(this.initialInfo)) {
            this.vehicleLoanDeedIndividual.patchValue(this.initialInfo);
        } else {
            this.fillForm();
        }
    }
    ...
    buildForm() {
        this.vehicleLoanDeedIndividual = this.formBuilder.group({
    ...
    fillForm() {
        this.vehicleLoanDeedIndividual.patchValue({
                ...
                grandFatherName: [!ObjectUtil.isEmpty(this.nepData.grandFatherName) ?
                    this.nepData.grandFatherName : ''],
                fatherName: [!ObjectUtil.isEmpty(this.nepData.fatherName) ?
                    this.nepData.fatherName : ''],
                husbandName: [!ObjectUtil.isEmpty(this.nepData.husbandName) ?
                    this.nepData.husbandName : ''],
                permanentDistrict: [!ObjectUtil.isEmpty(this.nepData.customerPermanentAddress) ?
                    this.nepData.customerPermanentAddress.district : ''],
                permanentMunicipality: [!ObjectUtil.isEmpty(this.nepData.customerPermanentAddress) ?
                    this.nepData.customerPermanentAddress.municipality : ''],
                permanentWard: [!ObjectUtil.isEmpty(this.nepData.customerPermanentAddress) ?
                ...
                currentDistrict: [!ObjectUtil.isEmpty(this.nepData.customerTemporaryAddress) ?
                    this.nepData.customerTemporaryAddress.district : ''],
                currentMunicipality: [!ObjectUtil.isEmpty(this.nepData.customerTemporaryAddress) ?
                    this.nepData.customerTemporaryAddress.municipality : ''],
                currentWard: [!ObjectUtil.isEmpty(this.nepData.customerTemporaryAddress) ?
    ...
    submit() {
        ...
        if (!ObjectUtil.isEmpty(this.cadData) && !ObjectUtil.isEmpty(this.cadData.cadFileList)) {
            this.cadData.cadFileList.forEach(singleCadFile => {
                if (singleCadFile.customerLoanId === this.customerLoanId && singleCadFile.cadDocument.id === this.documentId) {
                    flag = false;
                    singleCadFile.initialInformation = JSON.stringify(this.vehicleLoanDeedIndividual.value);
                }
            });
            if (flag) {
                const cadFile = new CadFile();
                const document = new Document();
                cadFile.initialInformation = JSON.stringify(this.vehicleLoanDeedIndividual.value);
    ...
    changeToNepAmount(event: any, target, from) {
        this.vehicleLoanDeedIndividual.get([target]).patchValue(event.nepVal);
        this.vehicleLoanDeedIndividual.get([from]).patchValue(event.val);
    }

    patchFunction(target) {
        const patchValue1 = this.vehicleLoanDeedIndividual.get([target]).value;
        return patchValue1;
    }
}
