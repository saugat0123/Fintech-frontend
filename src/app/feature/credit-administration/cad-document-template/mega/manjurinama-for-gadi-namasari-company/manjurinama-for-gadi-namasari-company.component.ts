...
@Component({
  selector: 'app-manjurinama-for-gadi-namasari-company',
  templateUrl: './manjurinama-for-gadi-namasari-company.component.html',
  styleUrls: ['./manjurinama-for-gadi-namasari-company.component.scss']
})
export class ManjurinamaForGadiNamasariCompanyComponent implements OnInit {
  ...
  buildForm() {
    this.manjurinamaForGadiNamasariCompany = this.formBuilder.group({
      date : [undefined],
      chasisNo: [undefined],
      engineNo: [undefined],
  ...
  submit() {
    ...
    if (!ObjectUtil.isEmpty(this.cadData) && !ObjectUtil.isEmpty(this.cadData.cadFileList)) {
      this.cadData.cadFileList.forEach(singleCadFile => {
        if (singleCadFile.customerLoanId === this.customerLoanId && singleCadFile.cadDocument.id === this.documentId) {
          flag = false;
          singleCadFile.initialInformation = JSON.stringify(this.manjurinamaForGadiNamasariCompany.value);
        }
      });
      if (flag) {
        const cadFile = new CadFile();
        const document = new Document();
        cadFile.initialInformation = JSON.stringify(this.manjurinamaForGadiNamasariCompany.value);
