...
export class AssignmentOfReceivablePartnershipComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (!ObjectUtil.isEmpty(this.cadData.offerDocumentList) && !ObjectUtil.isEmpty(this.cadData.offerDocumentList[0])) {
    ...
    this.companyInfo.forEach(val => {
      ...
      if (val.ownerNationality === 'Other') {
        ...
        if (val.otherOwnerPassportValidityDateOption === 'AD') {
          ...
          date2 = val ? val.otherOwnerPassportValidityDateNepaliCT.nDate : '';
        }
        const newDate = {
          issueDate: date,
          validDate: date2
