...
export class LoanDeedMultipleComponent implements OnInit {
  ...
  fillForm(): void {
    ...
    if (!this.existingOfferLetter) {
      ...
      this.customerOfferLetterService.detail(this.loanDataHolder.customerOfferLetter.id).subscribe(response => {
        ...
        this.loandeedmultiple.patchValue(initialInfo, {emitEvent: false});
    ...
    this.loandeedmultiple.valueChanges.subscribe((value) => {
      this.loandeedmultiple.patchValue(value, {emitEvent: false});
    });
  }
  ...
  submit(): void {
    ...
    if (this.existingOfferLetter) {
      ...
      customerOfferLetterPath.initialInformation = JSON.stringify(this.loandeedmultiple.value);
    ...
    this.customerOfferLetterService.save(this.customerOfferLetter).subscribe(() => {
      this.toastService.show(new Alert(AlertType.SUCCESS, 'Successfully saved Loan Deed Multiple borrower'));
      this.spinner = false;
      ...
      console.error(error);
      this.toastService.show(new Alert(AlertType.ERROR, 'Failed to save  Loan Deed Multiple borrower'));
      this.spinner = false;
    });
  }
}