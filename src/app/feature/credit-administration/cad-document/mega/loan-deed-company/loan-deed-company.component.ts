...
export class LoanDeedCompanyComponent implements OnInit {
  ...
  fillForm(): void {
    ...
    if (!this.existingOfferLetter) {
      ...
      this.customerOfferLetterService.detail(this.loanDataHolder.customerOfferLetter.id).subscribe(response => {
        ...
        this.loandeedcompany.patchValue(initialInfo, {emitEvent: false});
    ...
    this.loandeedcompany.valueChanges.subscribe((value) => {
      this.loandeedcompany.patchValue(value, {emitEvent: false});
    });
  }
  ...
  submit(): void {
    ...
    if (this.existingOfferLetter) {
      ...
      customerOfferLetterPath.initialInformation = JSON.stringify(this.loandeedcompany.value);
    ...
    this.customerOfferLetterService.save(this.customerOfferLetter).subscribe(() => {
      this.toastService.show(new Alert(AlertType.SUCCESS, 'Successfully saved Loan Deed Company'));
      this.spinner = false;
      ...
      console.error(error);
      this.toastService.show(new Alert(AlertType.ERROR, 'Failed to save  Loan Deed Comapny'));
      this.spinner = false;
    });
  }
}
