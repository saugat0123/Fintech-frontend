...
export class OverdraftLoanForWorkingCapitalRequirementComponent implements OnInit {
  ...
  async translateAndSetVal(index) {
    ...
    const baseRate = this.convertNumbersToNepali(this.overdraftLoanForm.get(['overdraftLoanFormArray', index, 'baseRate']).value.toFixed(2), false);
    this.overdraftLoanForm.get(['overdraftLoanFormArray', index, 'baseRateTrans']).patchValue(baseRate);
    const premiumRate = this.convertNumbersToNepali(this.overdraftLoanForm.get(['overdraftLoanFormArray', index, 'premiumRate']).value.toFixed(2), false);
