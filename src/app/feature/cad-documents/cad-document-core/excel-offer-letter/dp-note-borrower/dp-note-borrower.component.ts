...
export class DpNoteBorrowerComponent implements OnInit {
  ...
  ngOnInit() {
    console.log(this.dpnoteborrower);
    this.buildForm();
    this.fillForm();
  }

  buildForm() {
    this.dpnoteborrower = this.formBuilder.group({
      borrower: [undefined],
      name: [undefined],
      amount: [undefined],
  ...
  fillForm(): void {
    ...
    if (!this.existingOfferLetter) {
      ...
      this.customerOfferLetterService.detail(this.loanDataHolder.customerOfferLetter.id).subscribe(response => {
        ...
        this.dpnoteborrower.patchValue(initialInfo, {emitEvent: false});
    ...
    this.dpnoteborrower.valueChanges.subscribe((value) => {
      this.dpnoteborrower.patchValue(value, {emitEvent: false});
    });
  }
  ...
  submit(): void {
    ...
    if (this.existingOfferLetter) {
      this.customerOfferLetter.customerOfferLetterPath.forEach(offerLetterPath => {
        if (offerLetterPath.offerLetter.id === this.offerLetterTypeId) {
          offerLetterPath.initialInformation = JSON.stringify(this.dpnoteborrower.value);
        }
      });
    } else {
      ...
      customerOfferLetterPath.initialInformation = JSON.stringify(this.dpnoteborrower.value);
