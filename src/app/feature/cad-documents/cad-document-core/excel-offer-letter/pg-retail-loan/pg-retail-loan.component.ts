...
export class PgRetailLoanComponent implements OnInit {
  ...
  ngOnInit() {
    console.log(this.pgretailloan);
    this.buildForm();
    this.fillForm();
  }

  buildForm() {
    this.pgretailloan = this.formBuilder.group({
      address: [undefined],
      name: [undefined],
      amount: [undefined],
  ...
  fillForm(): void {
    ...
    if (!this.existingOfferLetter) {
      ...
      this.customerOfferLetterService.detail(this.loanDataHolder.customerOfferLetter.id).subscribe(response => {
        ...
        this.pgretailloan.patchValue(initialInfo, {emitEvent: false});
    ...
    this.pgretailloan.valueChanges.subscribe((value) => {
      this.pgretailloan.patchValue(value, {emitEvent: false});
    });
  }
  ...
  submit(): void {
    ...
    if (this.existingOfferLetter) {
      this.customerOfferLetter.customerOfferLetterPath.forEach(offerLetterPath => {
        if (offerLetterPath.offerLetter.id === this.offerLetterTypeId) {
          offerLetterPath.initialInformation = JSON.stringify(this.pgretailloan.value);
        }
      });
    } else {
      ...
      customerOfferLetterPath.initialInformation = JSON.stringify(this.pgretailloan.value);
