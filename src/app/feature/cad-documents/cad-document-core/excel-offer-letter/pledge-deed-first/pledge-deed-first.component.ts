...
export class PledgeDeedFirstComponent implements OnInit {
  ...
  ngOnInit() {
    console.log(this.pledgedeedfirst);
    this.buildForm();
    this.fillForm();
  }

  buildForm() {
    this.pledgedeedfirst = this.formBuilder.group({
      address: [undefined],
      name: [undefined],
      amount: [undefined],
  ...
  fillForm(): void {
    ...
    if (!this.existingOfferLetter) {
      ...
      this.customerOfferLetterService.detail(this.loanDataHolder.customerOfferLetter.id).subscribe(response => {
        ...
        this.pledgedeedfirst.patchValue(initialInfo, {emitEvent: false});
    ...
    this.pledgedeedfirst.valueChanges.subscribe((value) => {
      this.pledgedeedfirst.patchValue(value, {emitEvent: false});
    });
  }
  ...
  submit(): void {
    ...
    if (this.existingOfferLetter) {
      this.customerOfferLetter.customerOfferLetterPath.forEach(offerLetterPath => {
        if (offerLetterPath.offerLetter.id === this.offerLetterTypeId) {
          offerLetterPath.initialInformation = JSON.stringify(this.pledgedeedfirst.value);
        }
      });
    } else {
      ...
      customerOfferLetterPath.initialInformation = JSON.stringify(this.pledgedeedfirst.value);
