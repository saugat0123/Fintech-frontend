...
@Component({
  selector: 'app-hsov-pending',
  templateUrl: './hsov-pending.component.html',
  styleUrls: ['./hsov-pending.component.scss']
})
export class HsovPendingComponent implements OnInit {
  ...
  static loadData(other: HsovPendingComponent) {
    other.spinner = true;
    other.toggleArray = [];
    other.loanForCombine = [];
    other.loanHolderLoanList = [];
    other.loanHolderLoanListTemp = [];
    other.search.documentStatus = 'HSOV_PENDING';
    other.search.toUser = LocalStorageUtil.getStorage().userId;
    other.loanFormService.getHsovPull(other.search, other.page, 10).subscribe(
        (response: any) => {
          other.spinner = false;
  ...
  ngOnInit() {
    ...
    HsovPendingComponent.loadData(this);
  ...
  onSearch() {
   ...
   this.search.documentStatus = 'HSOV_PENDING';
    this.search.toUser = LocalStorageUtil.getStorage().userId;
    this.search.loanNewRenew = ObjectUtil.isEmpty(this.filterForm.get('documentStatus').value) ? this.initStatus :
